...
export const ja: LpCopy = {
  ...
  trust: {
    ...
    stars: {
      label: "GitHub のスター",
      ask: "このツールが役に立ったら、GitHub でスターを付けてもらえると嬉しいです。",
      button: "スターを付ける",
      why: "ZIP版の警告を消すためのコード署名証明書には、OSS向けに無償提供される制度がありますが、「検証可能な一定の評判」が条件となります。GitHub のスターは、その最も分かりやすい指標です。ワンクリックの応援が、次に ZIP 版をダウンロードする誰かの助けになります。",
    },
  },
  steps: {
    eyebrow: "クイックスタート",
