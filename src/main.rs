    ...
    fn take_events(&mut self, active_tab: Option<&Tab>) {
        ...
        for ev in self.win.drain() {
            match ev {
                ...
                // The tool page's question. Answered on a thread of its own:
                // reading a picture is an AI started and waited for, and the
                // loop draws the terminal meanwhile
                Ev::SnipAsk { msg } => {
                    let reply = self.win.snip_replier();
                    std::thread::spawn(move || {
                        let v = serde_json::from_str::<serde_json::Value>(&msg).unwrap_or_default();
                        reply.answer(shikisha_core::snip::answer(&v).to_string());
                    });
                }
