...
window.probePrepare = kind => {
  ...
  const desk = {id:"sample", name:"Sample desk", folders:[{cwd:""},{cwd:"branch",name:"Shipping labels"}],
    tabs:[newTab({id:"sample",name:"Sample tab",command:"cmd.exe",group:0})]};
  desks = [desk]; sel = {desk:0,tab:null,global:false}; loadFailure = null;
  ...
  if (kind === "desk" || kind === "desk-secrets") {
    if (kind === "desk-secrets") probeSecrets = [{key:"sample.password"}];
    add(deskPane(desk)); key = "settings.desk.delete";
  }
  if (kind === "folder") { add(folderPane(desk, desk.folders[1], 1)); key = "settings.group.discard"; }
  if (kind === "secret") {
    secretDialog(desk, {key:"sample.password",short:"password",urls:[]}); key = "common.delete";
  }
  if (kind === "tab") { sel.tab = 0; add(tabPane(desk, desk.tabs[0])); key = "settings.tab.delete"; }
  if (kind === "close") {
  ...
  window.probeState = () => JSON.stringify({writes:probeWrites, providers:current.providers,
    notify:current.notify, desks:desks.length, tabs:desk.tabs.length, folders:desk.folders.length});
  return key;
};
