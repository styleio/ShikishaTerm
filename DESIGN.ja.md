...
| **デスク** | 切り替える単位。仮想デスクトップ相当で、複数タブをまとめたもの（例: ProjectX、雑用）|
| **デスク定義ファイル** | デスクの中身 (タブ定義) を外部化したJSON。`desks/*.json`。コピー・共有できる単位 |
| **プロファイル** | ツール毎の状態検出ルール。`profiles/*.json` |
| **フック** | 状態遷移で発火するLuaスクリプト。`scripts/*.lua` |
| **チェーン深度** | 自動送信が何回連鎖したかのカウンタ。人間の手動入力で0に戻る |

config.json の `desks` は**デスクの一覧（目次）**で、各項目は中身を
...
| 端末制御シーケンス | ベル文字(BEL)、ウィンドウタイトル(OSC 0/2 — ターン中にCLIが書く印。プロファイルの `title_busy` に列挙)、代替スクリーン切替、カーソル表示/非表示 | 中 |
| 出力沈黙タイマー | N秒間出力なし＋カーソル入力位置 → 入力待ちとみなす | 中（汎用フォールバック） |
| プログラム自身の申告 | CLI自身のフックが、始めたターン・開いたダイアログ・終えたターンをその瞬間に報告する（`--hook state:BUSY`。設定画面からCLIごとに導入） | 確実（持っているCLIなら） |
| プロセス終了 | 子プロセスのexitコード | 確実 |

プログラム自身の申告があるときは、それが他のすべてに優先する。これは推測ではない——CLIがターンの
開始と終了をその瞬間に、どの言語でも、どのテーマでも、こちらが知っている単語をたまたま
描いたかどうかに関係なく発火させる。2秒の沈黙タイマーが「止まっただけのターン」を
終わらせてしまう癖は、これで消える。どのCLIも報告しないのは許可ダイアログへの*答え*で、
承認・拒否・Ctrl+C は人がCLIに対してすることであって、CLIがすることではない。だから
立ち上がりはフックが、立ち下がりは画面が受け持ち、来ないイベントに依存させない。
画面に見つけた問いは依然として最優先である。CLIが出す問いのすべてが報告されるわけでは
なく、人を待っているのに「作業中」と言っているタブは、誰も見に戻らない唯一の間違いだから。

タイトルはその中間に置く。申告より下なのは、フックはCLI自身だがタイトルはそのタブで動いて
いる何もが書けるから。画面より上なのは、考えているだけのCLIは何も描かず、2秒動かない画面が
「終わったターン」と読まれて——まだ作業中のAIから仕事を引き取ってしまっていたから。
言えるのは「ターンが走っている」だけである。どの種類の休みか（終わった・入力待ち・人待ち）は
受け渡しが依存するところで、それをタイトルに書くCLIは無い。
...
Lua環境は**デスク単位**で1つ。各スクリプトは独立した名前空間で読まれるので
`on_done` を複数ファイルで定義しても衝突せず、共有変数 (`get_var`/`set_var`) は
デスク内で共有される (A⇔Bループの往復カウンタ等が成立する)。
...
ものである（AI生成コード・共有されたデスク定義・ネットで拾った自動化）。
...
## 10.5 設定のホットリロード
設定変更のたびにアプリを再起動させない。設定ファイル・デスク定義・
...
デスクはタブの並びだけではない。タブが指す自動化スクリプトが別の場所にあり、
...
- 別ファイルに切り出したデスク (`file`) も、その場に展開して1枚で完結させる
...
[SHIKISHA-TERM Directory]
 ...
 ├── config.json            # 全体設定 + デスク一覧（相対パス）
 ├── secrets.json           # 資格情報（暗号化可・共有厳禁）
 ├── desks/            # デスク定義ファイル（プロジェクト毎に配布可）
 │     ├── projectx.json
 │     └── zatsuyo.json
