...
| `tools/debug/ideas.win.mjs` | Windows | The ideas through the running app's own window: this checkout's build started in a folder of its own with two project folders, and the ideas used as a person does — the bulb, typing, Enter and Shift+Enter, an edit, ticking one off, carrying one by its grip, a card with no project, and a project taken out of the settings. Every step is checked against `config/ideas.json` itself | `cargo build`, then `node tools/debug/ideas.win.mjs` |
