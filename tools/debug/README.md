...
| `tools/debug/type-from-elsewhere.win.mjs` | Windows, with Chrome | Whether somebody looking at the board from another machine can simply type. This checkout's build started in a folder of its own with the relay on, then opened in a headless Chrome twice: as a laptop (keys pressed with nothing clicked must reach the terminal, caret in the pane as at the window) and as a touch screen (a tap must bring the sub-input bar up and must NOT pull the caret into the pane, the bar's ⌨ must hand typing back to the screen and the ✎ pen take it back). The fault it was written for was invisible from here: the page simply had no caret anywhere, so keys went nowhere, with no error and nothing in any log. A soft keyboard cannot be stood in for -- keys sent over DevTools arrive as a real keyboard's do | `cargo build`, then `node tools/debug/type-from-elsewhere.win.mjs` |
