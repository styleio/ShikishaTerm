...
ここまでの設定は、どのデスクにも効くアプリ共通の答えです。会社の
リポジトリと自分のものを1台で扱うような場合は、`desks` の中に書けば、
そのデスクだけの答えになります。
