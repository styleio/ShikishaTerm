...
**同じ7つを、画面から。** コマンドが `sftp <サーバーのタブの呼び名>` のタブは、ファイルのパネルです。
左にこのフォルダ、右にそのサーバーの、2枚の一覧が出ます。画面が持っている手立ては上の命令だけで、
権限も同じ表を見ます。手でできることと台本にできることが食い違わないためです。接続はパネルの上で
選びます。その接続の中身はサーバーのタブ自身にあります。
