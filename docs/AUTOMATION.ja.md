...
窓口・自動化の権限・通知先・AIプロバイダ・git の設定は、**デスクごとに持ちます**。
...
設定（ブラウザ ›「判断モデル」「会話モデル」）が決めます。そこで何も選んでいなければ、
基本設定のアシスタントAIが答えます。
...
| `shikisha.ai_ask("聞きたいこと")` | 基本設定の**アシスタントAI**に聞いて、答えの文字列を返す。答えが得られなければ `nil` と理由。**待っている間もアプリは止まりません**（`sleep` と同じ仕組みで、他のタブも画面も動き続けます）。既定の待ち時間は3分、`{timeout_ms=…}` で変えられる。`{light=true}` にすると、そのAIでいちばん安い聞き方で短い答えをもらう（選べるなら最小のモデル、道具なし、長い指示なし）。1〜2行より長い答えがほしいときは付けない。`{ai="codex"}` で別のアシスタントAIに、`{ai="model deepseek/deepseek-chat"}` でこのデスクのAIプロバイダに聞く |
