...
窓口・自動化の権限・通知先・git の設定は、**デスクごとに持ちます**。
アプリ全体の設定というものはなく、デスクに書いていないものは「無い」という意味です
（窓口なし、標準の権限、通知先なし、組み込みの git 設定）。
会社のリポジトリと自分のものを1台で扱っても、片方の通知先が
もう片方から使われることはありません。AIプロバイダだけは例外で、アプリ全体で1つの一覧
（設定 › AIエージェント）を持ち、どのデスクのタブとスクリプトからも名前で使えます。
...
git アカウントはアプリのもので、ファイルの一番上に接続先と並べて書きます
（`"git_accounts": [ { "name": "work", "login": "me-at-work", "owners": ["my-company"] },
{ "name": "home", "method": "ssh", "key": "C:/Users/me/.ssh/id_home" } ]`）。どのデスクの
プロジェクトも同じ一覧から選びます。トークンはここには書きません。全体設定の
「Git アカウント」で入れると、`git/<アカウント名>` という名前で保存されます。
git 欄でサインインするアカウントは、プロジェクトが `git_account` で選び、git タブは
自分で選びます。書いていなければ、この PC の git がいつもどおりにサインインします。
`"@pc:<ログイン名>"` は、この PC の git が保存している GitHub のアカウントの1つを名指しする
書き方で、2つ保存されているときに使います。`"@gh:<サーバー>/<ログイン名>"` は、GitHub CLI（gh）が
サインインしているアカウントの1つを名指しする書き方です。プルリクエスト番号も同じアカウントで
...
Model Context Protocol を話すAIクライアント（Claude Code など）には、ここの命令を
そのまま道具として渡せます。
...
タブの中のCLIが起動したときは、これだけで繋がります。そのタブの合言葉が既に環境変数に
入っているので、呼び出しは**そのタブからのもの**として届き、そのタブの連鎖の上限と権限に
数えられます。別の起動中のコピー（あなたが作業している方ではなく、試している方）へ
向けるときは、そのコピーを名指しします。
...
`--pid` はそのコピーのプロセスID（パイプの名前がそれを持っています）、`--token-file` は
`access: "user"` が合言葉を置く場所です。`--pipe` と `--token` なら直接書けます。

道具の名前は9章の命令に `shikisha_` を付けたものです（このアプリの `send` が、別のサーバーの
`send` と混ざらないため）。引数は命令が取る順に `params` へ並べます。

```jsonc
{ "name": "shikisha_send_to_tab", "arguments": { "params": ["reviewer", "状況は?"] } }
```

道具の一覧は `list` の答えそのもので、毎回、同じ入口と同じ権限で動いているアプリに尋ねます。
**クライアントに見えるものと、アプリが実際にできることは食い違いません。** 誰が尋ねたかで
変わる部分も同じです。デスクの設定でAIに対して切ってある命令は、AIに渡される一覧には出ません。

断られた命令は、接続が壊れたのではなく「失敗した道具」として理由つきで返ります。
モデルはそれを読んで別の手を試せます。
...
どちらの列も、既定ではほとんど入っています。**AIに対して既定で閉じているのは次の命令**で、
どれも「この表そのものを回り込める」「あなたの持ち物を壊せる」「あなたの名前で発言する」のどれかです。

| 命令 | 既定でAIに閉じている理由 |
|---|---|
| `lua` | 囲いなしでコードを実行できる。開ければこの表が意味を持たなくなる |
| `read_path` / `list_path` / `write_path` / `http_raw` | 窓口を通さない生のパス・URL。許可フォルダと許可ホストは、あなたが自分のスクリプトのために開けたもの |
| `open_tab` | タブは設定の1行に書いたものを何でも動かすので、タブを開くことはプログラムを起動することと同じ。この表のどの行にも縛られない端末を1つ持たせることになる |
| `close_tab` / `close_pane` | タブの中で動いているプログラムを終わらせる／人が見ていた場所を消す |
| `restart` | 会話ごとタブを作り直す |
| `sftp_mkdir` / `sftp_rename` / `sftp_rm` | 別のマシンのファイルを変える。向こう側には元に戻す手段も予備の写しもない |
| `git_` と `github_` で始まる命令すべて | あなたが選んだ git のアカウントや GitHub のアカウントとして動き、書き込んだものはそのアカウントの名前で出る |
| `reply_url` | そのタブに文字を打てるリンクを、リンクに届く人すべてに渡すことになる |
| `ai_ask` | お金と時間がかかる。AI に AI を呼ばせると、繰り返し呼び続けられる |
...
| `shikisha.open_tab({ command = "codex", name = "調査" })` | **タブを追加する。** 書き方は設定ファイルのタブ1行と同じで、`command` がタブで動かすもの、`name` や `id` などは設定と同じキーです。タブは呼んだタブと同じフォルダに入り、`folder` を指定すればそのフォルダに入ります（デスクに既にあるフォルダだけ。2台のマシンに同じパスのフォルダがあるときは、`host` に設定上のマシン名を書いてどちらかを指定します）。戻り値は `{ id = ..., folder = ..., host = ... }` で、タブに付いた自動化の名前（`id` を省けば自動で付きます）と、入ったフォルダとマシンです。デスク内の別のタブが使っている `id` を指定すると断ります。タブが起動する前に新しい id へ宛てた命令（`send_to_tab`・`show`・`close_tab` など）は、タブが現れるのを待ってから実行されます |
| `shikisha.close_tab(タブ)` | タブを閉じる。✕ を押したときと同じ動きで、AI が処理中か確認待ちのタブはその場では閉じず、人に確認します |

**別の AI に調べものを頼む。** 言う順番どおりに命令を並べます。

```lua
local t = shikisha.open_tab({ command = "codex", name = "調査" })
shikisha.send_to_tab(t.id, "ビルドが遅い原因を調べて")  -- タブの起動を待ってから届く
...
「タブを開いて指示を打つ」を1つにまとめた命令はありません。「画面を分けてブラウザを置く」命令が無いのと
同じ理由です。タブの種類はどれも `command` の書き方で決まり、使い方はどれもこれらの命令の順番で決まります。
...
設定（ブラウザタブの「判断モデル」「会話モデル」、選んでいない方は AIエージェント の
「判断AI」）が決めます。そこで何も選んでいなければ、アシスタントAIが答えます。

この PC に入っている AI も答えられます。ログインしている契約で動き、API キーは
要りません: `model = "@claude"`、`"@codex"`、`"@gemini"`。モデルを指定するときは
スラッシュの後に書きます（`"@claude/haiku"`）。道具を持たせず、空のフォルダで
聞くので、会話モデルと同じように答えるだけです。

**モデルが考えている間も、アプリは動き続けます**（フックとクイックアクションの中）。
どちらも `sleep` や `ai_ask` と同じく聞いた所で止まり、ほかのタブや画面はそのまま
動きます。`browser_settle` も同じです。AI がページ用に書いた Lua は途中で止まらずに
最後まで走るので、待つ間はその場で待ちます。
...
| `shikisha.ai_ask("聞きたいこと")` | 設定 > AIエージェント の**アシスタントAI**に聞いて、答えの文字列を返す。答えが得られなければ `nil` と理由。**待っている間もアプリは止まりません**（`sleep` と同じ仕組みで、他のタブも画面も動き続けます）。既定の待ち時間は3分、`{timeout_ms=…}` で変えられる。`{light=true}` にすると、そのAIでいちばん安い聞き方で短い答えをもらう（選べるなら最小のモデル、道具なし、長い指示なし）。1〜2行より長い答えがほしいときは付けない。`{ai="codex"}` で別のアシスタントAIに、`{ai="model deepseek/deepseek-chat"}` で登録したAIプロバイダに聞く |
...
| `shikisha.git_discard(タブ, パス, {staged=…, plan=…})` | そのファイルの変更を破棄する。`staged` を渡さないと、次のコミットに入っていない分だけが消え、git がまだ知らないファイルはディスクから削除される。渡すと、ファイルは直前のコミットの状態に戻り、直前のコミットに無いファイルは次のコミットから外れてディスクから削除される。衝突しているファイルには触れない。**これは元に戻せない**（消すのはコミットしていない変更なので、git のどこにも残らない）。`plan=true` なら何も実行せずに答えるので、聞く前に「何が走るか」を画面に出せる。どちらも答えは `{plan, said}` で、`said` はそのコマンドを人が打つときの形で並べたもの |
...
| `shikisha.git_run(タブ, "引数…")` | 任意の git を実行して、その出力を返す。**シェルは通りません**（`;` や `&&` は git の引数になって断られる）。選ばれた git アカウントでサインインし、選ばれていなければこの PC の git がいつもどおりにサインインする |

**サインインするアカウント。** 何も選んでいなければ、この PC の git がいつもどおりにサインインします（git 自身の資格情報ヘルパーと鍵）。ほとんどの人はそれで動き、意識することはありません。git タブは、そのタブのページで選んだアカウントを使います。ほかのタブは、作業フォルダのプロジェクトがプロジェクトのページで選んだアカウントを使います。アカウント自体はアプリに登録します（全体設定 → Git アカウント）。HTTPS のトークンか SSH の鍵ファイルと、コミットに付く名前とメールを持ち、名前とメールは `git_commit` と `git_merge` でも使います。同じページには、この PC の git と GitHub CLI（gh）が保存している GitHub のサインインも並び、プロジェクトはそのどれかを名前で選べます（`@pc:<ログイン名>`）。この PC に GitHub のアカウントが2つ保存されていると git はどちらを使うか決められないので、そのときに選びます。取得・プル・プッシュがアカウントの理由で断られると、git 欄がそう言い、そのページへのボタンを出します。ターミナルのタブで打った git も、同じアカウントでサインインします。タブを起動するときにその設定（`GIT_CONFIG_*` と、そのアカウントのサーバーに対する資格情報ヘルパー）を渡すので、そのタブで動くプログラム（AI を含む）も同じアカウントとして git を実行します。取り上げるものはなく、ほかのサーバーのリポジトリは、この PC がこれまでどおりの方法でサインインします。設定の変更は、そのタブを次に開いたときから効きます。
