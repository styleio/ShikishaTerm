...
**フォルダごと送る命令はありません。** 送るときは `sftp_ls_here` と `sftp_put`、取り寄せるときは `sftp_ls` と `sftp_get` を繰り返して書きます。1つの命令にすると、
...
| `shikisha.ai_ask("聞きたいこと")` | 基本設定の**アシスタントAI**に聞いて、答えの文字列を返す。答えが得られなければ `nil` と理由。**待っている間もアプリは止まりません**（`sleep` と同じ仕組みで、他のタブも画面も動き続けます）。既定の待ち時間は3分、`{timeout_ms=…}` で変えられる |
