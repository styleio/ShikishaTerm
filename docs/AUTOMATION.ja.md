...
```lua
local seed = "lp.html を読んでください。

"
shikisha.send("ai", "[200~" .. seed .. "[201~")
```

`[200~` と `[201~` はペーストの始まりと終わりの目印です。この内側の改行は
**キーを押したのではなく文字**として扱われます。貼り付けたスクリプトが、貼った先から
1行ずつ勝手に走り出すのを防ぐために端末が持っている仕組みです。AI CLI はこれを尊重
しますし、このソフト自身も送信と見なさないので、そのタブで `on_done` が撃たれません。

**素のシェルは尊重しません。** `cmd.exe` で実測したところ、目印は丸ごと無視され、
間に入れた復帰(CR)はそのまま実行されました。**下書きはAI CLIに置くもので、
シェルにコマンドを仕込むのに使ってはいけません。**
