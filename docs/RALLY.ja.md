...
```
全拒否(既定) ── デスクで選択(通常) ── 全許可トグル(危険承知)
   安全 ←─────────────────────────────────→ 便利
```

- デスク設定に「使える秘密」チェックリスト（キー＋説明が並ぶ）
- 「全許可（危険）」トグルを警告付きで用意
- ラリーはそのデスクが許可した秘密名しか参照できない
  （別用途の鍵の流用を封じる）
...
- **P7 参照司令塔＋ドキュメント**：`on_start.lua`/`on_done.lua` の雛形と、
  記録/再生・秘密・許可リストの手順書。デスクに載せれば動く形。
...
| P4b 遮蔽 `redact` | ✅ |
| P4c 秘密登録GUI | ✅ 実機検証済み |
| P4d 許可リスト＋`browser_fill_secret` | ✅（デスク許可リストのGUIは未実装、config直書きで有効） |
...
`docs/rally-example/` に置いた3ファイルを、デスクの **AIセッションタブ** の
...
1. **秘密を登録**（必要なら）：設定GUIの「秘密」で `example_login` 等を登録（`user:pass` 形式なら
   ベーシック認証にも使える）。
2. **デスクを用意**：AIセッションタブ（例 `claude`）＋ブラウザタブ（例 id=`br`）。
