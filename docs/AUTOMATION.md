...
| `shikisha.notify("target", "text")` | Notify Slack / Discord / Telegram, or this PC's own notification area (only configured targets) |
...
| `shikisha.notify("text")` / `shikisha.notify("target", "text")` | Notify Slack / Discord / Telegram, or this PC's own notification area (only targets you configured). With no target named, it goes to the default one |
