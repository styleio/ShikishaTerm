...
| `shikisha.git_branch(tab)` | The branch: `{name, protected, upstream, ahead, behind}`. `protected` marks one this folder guards, so committing straight onto it is worth asking about. `upstream` is the branch it follows (`origin/main`), `ahead` the commits here not there yet and `behind` the other way round, as of the last fetch -- all three absent when it follows nothing. `nil` when the head is detached |
