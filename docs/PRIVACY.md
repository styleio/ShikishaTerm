...
| The tools' **AI text reading and AI naming**, only on a desk that agreed to them | The part of the screen you framed, as a picture, handed to the AI command-line tool chosen under Settings › Basic › Assistant AI (Claude Code, Codex or Gemini), which sends it to its own provider. The agreement is saved in the desk's settings together with that AI, and is asked for again if the assistant AI changes. The picture is deleted from the local temporary folder once it has been read. |
