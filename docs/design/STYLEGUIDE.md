...
| **Sidebar** (settings) | What is being edited | From the larger world down: **General ›** · the **desk** (initial plate · name · `▾`) · **Desk settings ›** · the tree of folders and tabs. The two `›` rows are plain buttons (`--edge` rim on `--panel2`, 32px, `--r-ctl`) and each **enters a place**: the column is replaced by `‹ Back` · a heading · one entry per card (name, and one line under it). `‹ Back` returns to the tree. The desk's `▾` lists the desks; choosing one opens that desk's settings |
