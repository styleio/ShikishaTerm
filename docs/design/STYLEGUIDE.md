...
| **Speed tag** (`.speedchip`) | How quickly a page is driven in words (whether a decision model decides) | One grey tag: 11px `--dim` on `--panel2`, `--line` border, 4px corners. "Fast" when a decision model decides, and not pressable. Otherwise "Slow", pressable (`--text` under the pointer): the press puts the reason and the fix under it in a `--warn` box, carries the view to the Decision model field and lights it (`.lookhere`, the glow that says where). Speed is said in words, not colour: both are grey |
