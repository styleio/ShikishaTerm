...
impl Row {
    ...
    pub fn insert(&mut self, i: u16, cell: crate::Cell) {
        // NOTE (vendored patch): a resize can hand us a column past the end
        let i = usize::from(i).min(self.cells.len());
        self.cells.insert(i, cell);
        self.wrapped = false;
    }

    pub fn remove(&mut self, i: u16) {
        // NOTE (vendored patch): a resize can hand us an out-of-range column
        if usize::from(i) >= self.cells.len() {
            return;
        }
        self.clear_wide(i);
    ...
    pub fn erase(&mut self, i: u16, attrs: crate::attrs::Attrs) {
        // NOTE (vendored patch): a resize can hand us an out-of-range column,
        // and `cols() - 2` underflows on a 1-column screen
        let Some(cell) = self.cells.get(usize::from(i)) else {
            return;
        };
    ...
    pub fn truncate(&mut self, len: u16) {
        ...
        self.wrapped = false;
        // NOTE (vendored patch): len 0 used to underflow
        let Some(idx) = usize::from(len).checked_sub(1) else {
            return;
        };
    ...
    pub fn clear_wide(&mut self, col: u16) {
        // NOTE (vendored patch): a resize can leave a wide flag with no
        // partner cell (or a continuation flag in column 0); skip instead of
