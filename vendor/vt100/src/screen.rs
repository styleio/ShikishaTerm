...
impl Screen {
    pub(crate) fn text(&mut self, c: char) {
        ...
        let mut wrap = false;
        // NOTE (vendored patch): `size.cols - width` underflows when the
        // screen is narrower than the character (a 1-column terminal fed a
        // full-width char), and the last-cell lookup can miss after a
        // resize. Both degrade to "don't wrap" instead of panicking
        if width <= size.cols && pos.col > size.cols - width {
            if let Some(last_cell) = self.grid().drawing_cell(crate::grid::Pos {
                row: pos.row,
                col: size.cols - 1,
            }) {
        ...
        // NOTE (vendored patch): the original unwrap()ed every neighbor
        // lookup below, justified by invariants like "a wide cell always has
        // its continuation right after it". A mid-stream resize breaks those
        // invariants (a full-width character stranded at the new row edge),
        // and the panics took the whole screen down. Each lookup now skips
        // the cosmetic half of the work when the neighbor does not exist.
        if width == 0 {
            if pos.col > 0 {
                let mut prev_pos = crate::grid::Pos {
                    row: pos.row,
                    col: pos.col - 1,
                };
                if self
                    .grid()
                    .drawing_cell(prev_pos)
                    .is_some_and(crate::cell::Cell::is_wide_continuation)
                    && prev_pos.col > 0
                {
                    prev_pos.col -= 1;
                }
                if let Some(prev_cell) = self.grid_mut().drawing_cell_mut(prev_pos) {
                    prev_cell.append(c);
                }
            } else if pos.row > 0 {
                let wrapped = self
