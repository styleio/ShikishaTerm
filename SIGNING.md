...
Paid certificates were not the answer either, and not because of ten dollars.
Handing this program out costs nothing — there is no server of ours, no account,
and nothing collected — and a subscription would be a bill that has to be met
every month. Meeting it eventually means taking it from the people using the
program, which is the one thing this is built not to do. The Store signs the
build for nothing, which covers most people who install it; the zip is what is
left over, and free certificates for open source are given on reputation rather
than on payment.
